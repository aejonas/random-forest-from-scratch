...
def predict_example_tree(tree, example):
    ...
    node = tree
    while not node["leaf"]:
        if example[node["feature_index"]] <= node["threshold"]:
            node = node["left"]
        else:
            node = node["right"]
...
def predict_tree(tree, features):
    ...
    for example in features:
        predictions.append(predict_example_tree(tree, example))
...
def bootstrap_sample(features, labels, rng):
    ...
    indices = rng.integers(0, len(features), len(features))
...
def train_forest(features, labels, num_trees=10, max_depth=10, min_samples_split=2, num_features_per_split=None, random_state=0):
    ...
    if num_features_per_split is None:
        num_features_per_split = round(np.sqrt(features.shape[1]))

    trees = []
    for _ in range(num_trees):
        features_sampled, labels_sampled = bootstrap_sample(features, labels, rng)
        feature_indices = feature_subset(features.shape[1], num_features_per_split, rng)

        tree = build_tree(features_sampled, labels_sampled, max_depth, min_samples_split, feature_indices)  
...
def combine_predictions(tree_predictions):
    ...
    labels, counts = np.unique(tree_predictions, return_counts=True, axis=0)
